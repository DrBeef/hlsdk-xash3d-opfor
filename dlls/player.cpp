...
void CBasePlayer::PlayerUse( void )
{
	   ...
	   !tr.fStartSolid && tr.flFraction == 1.0f)
	{
        ...
        if (highlightActionables &&
			// We don't want to highlight things like pushable boxes
            pObject->pev->movetype != MOVETYPE_PUSHSTEP &&
            // We don't want to highlight helpful people as "usable" either (even though they are)
            pObject->Classify() != CLASS_HUMAN_PASSIVE &&
...
void CBasePlayer::PreThink( void )
{
	...
	const bool isScopeEngaged = (pev->fov != CVAR_GET_FLOAT("vr_hmd_fov_x") && pev->fov != 0);
